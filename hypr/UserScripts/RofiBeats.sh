...
# Function to list playlists (directories) in the music directory
list_playlists() {
  find "$mDIR" -mindepth 1 -maxdepth 1 -type d -exec basename {} \;
}

# Populate local_music array with files from a selected playlist
populate_local_music() {
  ...
  done < <(find "$playlist_dir" -type f \( -iname "*.mp3" -o -iname "*.flac" -o -iname "*.wav" -o -iname "*.ogg" -o -iname "*.mp4" \))
}
...
play_local_music() {
  ...
  playlist=$(list_playlists | rofi -i -dmenu -config ~/.config/rofi/config-rofi-Beats.rasi -p "Select Playlist")

  if [ -z "$playlist" ]; then
    exit 1
  fi

  # Populate the local_music array with the selected playlist
  populate_local_music "$mDIR/$playlist"
